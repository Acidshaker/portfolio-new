...
export const ClientForm = ({
  ...
  const onSubmit = async (body: any) => {
    ...
    try {
      ...
      if (!isEdit) {
        const res = await clients.createClient(newData);
        console.log(res.data);
      } else {
        const res = await clients.updateClient(data.id, newData);
        console.log(res.data);
      }
      toast.success(`Cliente ${isEdit ? "actualizado" : "creado"} con éxito`);
      handleClose();
    } catch (err) {
      console.log(err);
    } finally {
